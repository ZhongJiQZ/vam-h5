        ...
        <div class="card-body">
          ...
          <div class="card-kvs">
            ...
            <div class="card-kv">
              ...
              <span class="kv-val">{{ formatCopyTradeTime(item, COPY_TRADE_TIME_FIELD.JOIN) }}</span>
            </div>
            <div class="card-kv">
              ...
              <span class="kv-val">{{ formatCopyTradeTime(item, COPY_TRADE_TIME_FIELD.EXIT) }}</span>
            </div>
            <div class="card-kv">
              <span class="kv-dot" style="background:#17ac74">
...
const dateArr = computed(() => {
  const t = formatCopyTradeTime(props.item, COPY_TRADE_TIME_FIELD.EXIT, 'YYYY-MM-DD HH:mm:ss')
  if (!t || t === '--') return ['--', '']
  const withoutTz = t.replace(/\s*\(UTC[^)]*\)\s*$/, '')
  const parts = withoutTz.replace(/-/g, '/').split(' ')
