...
<template>
    ...
    <div class="list">
      ...
      <div class="item">
        ...
        <div class="fw-num numItem">
          ...
          {{ formatLocal(recordListItem.params?.delegateTime || recordListItem.delegateTime) }}
        </div>
      </div>
      <!-- 委托总量 -->
      <div class="item">
  ...
  <div class="hisToryList" v-if="currentEntruset === 3">
    ...
    <div class="list">
      ...
      <!-- 平仓时间 -->
      <div class="item">
        <div>{{ _t18(`close_time`) }}</div>
        <div class="fw-num numItem">{{ formatLocalTime(recordListItem.dealTime) }}</div>
      </div>
       <!-- 开仓手续费   -->
       <div class="item">
        <div>{{ _t18(`open_fee`) }}</div>
 ...
 * 收益率
 * **新改的
     * 做多仓位（Long）：  收益率 = (最新价 - 开仓价) / 开仓价 × 杠杆
     * 做空仓位（Short）  收益率 = (开仓价 - 最新价) / 开仓价 × 杠杆
...
const sharedImg = (model, yieldHisValue) => {
  console.log(model)
  // 二维码链接
  // const shareLink = `${getOrigin()}/#/i&`;//https://xhqb.net
  const shareLink = "https://xhqb.net/#/i&"
