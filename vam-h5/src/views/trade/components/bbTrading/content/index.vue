  ...
  <div>
    <div class="content">
      ...
      <div class="content_right">
        ...
        <div class="rightSix">
            <div>{{ form.type == 0 ? _t18(`bb_trade`) : _t18(`bb_trade_sell`) }}</div>
            <div class="number fw-num">{{form.delegateAmount}} {{getSymbol(props.coinInfo.symbol)}}</div>
        </div>
        <div class="rightSix">
            <div>{{ _t18(`bb_order_fee`) }}</div>
            <!--  手续费率 props.coinInfo.feeRate/100 币种单价:props.coinInfo.amount  -->
            <div class="number fw-num"> 
              {{form.delegateFee}} {{getSymbol(props.coinInfo.symbol)}}</div>
        </div> 
        <!-- 占位 -->
...
const sliderChange = (val) => {
  ...
  if (Number(tempPrice)) {
    if (form.delegateType == 1 && form.type == 1) {
      ...
      form.delegateAmount = round6(form.turnover / props.coinInfo.amount) ;
...
const round6 = (v) => Math.round((Number(v) + Number.EPSILON) * 1e6) / 1e6
const fmt6 = (v) => Number(v).toFixed(6).replace(/\.?0+$/, '')
/**
 * 数量监听
 * type (0 买入 1卖出)
 * delegate_type 委托类型（0 限价 1 市价 2 止盈止损  3 计划委托）
 */
const countChange = () => {
  let val = form.count 
  if (form.count > availableBalance.value) {
      form.count = availableBalance.value;
  }
  if (form.delegateType == 1 && form.type == 1) {
      // 市价&&卖出
