...
const loadAppConfig = () => {
  ...
  window.__config = {
    _BASE_API: import.meta.env.VITE_APP_BASE_API || `https://${domainUrl}`,
    _BASE_WSS: import.meta.env.VITE_APP_BASE_WSS || `wss://${domainUrl}`,
    ...
    if (testPlatformData) {
      try {
        ...
        if (testPlatformData.domain) {
          __config._BASE_API = `https://${testPlatformData.domain}/api`
          __config._BASE_WSS = `wss://${testPlatformData.domain}/api`
        }
      } catch (error) {
 ...
 * 注册邀请码必填 配置(账号，邮箱)
 */
export const REGISTER_REQUIRED_ACTIVECODE = ['production']
 ...
 *  pc配置
 */
export const PLATFORM_PC = ['production']
