 ...
 * @returns 过滤当前币种
 */
export const formatCurrentcurrencyBak = (historyList, coin) => {
  let listArray = []
...
export const formatCurrentcurrency = (list = [], symbol = '') => {
  const target = String(symbol || '').toUpperCase()

  return (list || []).filter((it) => {
    // 你这里根据接口实际字段选一个
    const s1 = String(it.symbol || '').toUpperCase()
    const s2 = String(it.coinSymbol || '').toUpperCase()
    const s3 = String(it.coinSymbolName || '').toUpperCase()

    // 允许包含关系（比如 BTCUSDT 包含 BTC）
    return s1 === target || s2 === target || s3 === target || s1.includes(target) || s2.includes(target)
  })
}
