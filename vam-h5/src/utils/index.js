...
export const switchPlanform = (initFlag = false) => {
  ...
  const { _APP_PLATFORM_H5, _APP_PLATFORM_PC } = __config
  let h5Path = ''
  let pcPath = ''
  if (_APP_PLATFORM_H5 != undefined) {
    h5Path = _APP_PLATFORM_H5 || `http://${location.host}`
  }
  if (_APP_PLATFORM_PC != undefined) {
    pcPath = _APP_PLATFORM_PC || `http://pc.${location.host}`
  }
