...
function getSaleSmartlyAppEnv(appEnv) {
  if (appEnv) return appEnv
  if (typeof window !== 'undefined' && window.__config && window.__config._APP_ENV) {
    return window.__config._APP_ENV
  }
  return 'vam'
}
...
export function injectSaleSmartlyPlatformScript(appEnv) {
  ...
  const env = getSaleSmartlyAppEnv(appEnv)
...
/**
 * 打开客服：加载项目插件 → setLoginInfo(user_id + custom_fields_ext) → chatOpen
 */
export async function openCustomerService({ userInfo, preferWidget = true, appEnv } = {}) {
  ...
  try {
    const env = getSaleSmartlyAppEnv(appEnv)
    injectSaleSmartlyPlatformScript(env)
