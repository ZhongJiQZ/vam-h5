...
export const _t18 = (name, platform = [], i18n = null) => {
  const ins = i18n || i18nInstance; // 优先用传入的，否则用全局实例
  if (!ins || typeof ins.t !== "function") return name;

  if (platform.includes(__config._APP_ENV)) {
    return ins.t(`${__config._APP_ENV}_${name}`);
  }
  return ins.t(name);
};
...
export const _timeFormat = (time, expression = '', flag = false) => {
  ...
  const timeFormat = store.getters.timeFormat;
  // console.log('时间', time, mainStore.timezone, mainStore.timeFormat)
  let tempExpression = timeFormat
  if (flag) {
