...
export default {
  ...
  data() {
    return {
      ...
      assetsTabsList: [
        //资产列表
        {
          label: "assets.mingOrder",
          name: "mingOrder",//api/miningOrder/list
        },
        {
          label: "assets.inverestOrder",
          name: "order",
        },
        {
          label: "assets.storeDetail",
          name: "recharge",//api/recharge/list 充值记录
        },
        {
          label: "assets.withdrawDetails",
          name: "withdraw",//api/withdraw/list
        },
      ],
      rechargeTable: [],
      withdrawTable: [],
  ...
  watch: {
    ...
    "$route.query.option"(newOpt) {
      if (newOpt !== undefined) {
        const n = Number(newOpt);
        this.overViewTabIndex = Number.isFinite(n) ? n : 0;
      }
    },
  ...
  async mounted() {
    ...
    // 关键：从 url 同步 overview 子选项（0/1/2）
    const opt = this.$route.query.option;
    if (opt !== undefined) {
      const n = Number(opt);
      this.overViewTabIndex = Number.isFinite(n) ? n : 0;
    }
        ...
        getRecordList(
          ...
          if (param.sendTabValue) {
            if (param.sendTabValue == "recharge") {
              ...
              this.tableDataTotal = res.data.total;
            } else if (param.sendTabValue == "order") {
              this.orderTable = res.data.rows;
              this.tableDataTotal = res.data.total;
            }else {
        ...
        for (let i = 0; i < this.assetsTabsList.length; i++) {
          getRecordList(
            ...
            if (this.assetsTabsList[i].name == "recharge") {
              ...
              this.withdrawTable = res.data.rows;
              this.tableDataTotal = res.data.total;
            } else if (this.assetsTabsList[i].name == "order"){
              this.orderTable = res.data.rows;
              this.tableDataTotal = res.data.total;
            }else {
              this.mingOrderTable = res.data.rows;
