              ...
              <div class="exchange-depths ">
                ...
                <div class="exchange-depth-content">
                  <div class="depth-row ">
                    ...
                    <span class="depth-row__item">
                      {{ $t("trade.haveDealNum") }}({{ coinSymbolInfo.coinUpperCase }})
                    </span>
                  </div>
                  <div
            ...
            <div class="right_container">
              ...
              <div class="kline_box">
                <div class="exchange-kline">
                  <Candlestick
                    v-if="coinSymbolInfo && coinSymbolInfo.symbolUpperCase"
                    :key="queryType + '_' + (coinSymbolInfo.symbolUpperCase || coinSymbolInfo.coin)"
        ...
        <div class="content_r">
          ...
          <div class="top_quotes">
            ...
            <div class="quotes_box">
              ...
              <div class="content-data">
                <ul class="el-scrollbar__view">
                  <li
                    v-for="(item, index) in tabCoinList"
                    :key="index"
                    class="flex-between"
                  >
          ...
          <div class="bottom_deal">
            ...
            <div class="tabs_box flex-between c3">
              ...
              <span style="margin-left: -10px">{{ $t("trade.price") }}(USDT)</span>
              <span style="margin-left: -10px">{{ $t("trade.haveDealNum") }}({{ coinSymbolInfo.coinUpperCase }})</span>
            </div>
            <div class="deal_list">
              <ul class="el-scrollbar__view">
                <li
                ...
                >
                   <!-- <el-tooltip
                      effect="dark"
                      placement="top"
                      :content="String(_div(item.amount, item.price))"
                    > -->
                    ...
                    <div class="count fw-num" :title="String(formatCountDiv(item.amount, item.price))">
...
export default {
  ...
  watch: {
    ...
    handler(t) {
      const type = t || "sec";
      this.queryType = type;
      this.tabsValue = type;            // ✅ 关键：tab UI 跟路由一致
      this.symbol = this.$route.query.symbol;

      this.syncTableListByType(type);   // ✅ 关键：列表先切
      this.initCoinInfo(type);          // 再初始化 coinSymbolInfo 等
    }
    },
    '$route.query.symbol': {
      immediate: true,
      handler(s) {
        this.symbol = (s || "btc").toLowerCase();
        // symbol 变动时不改 tableList，只要更新 coinSymbolInfo
        this.initCoinInfo(this.$route.query.type || "sec");
      }
    },
    countDown(val) {
      if (val) {
    ...
    secondContractCoinList(list) {
      if ((this.queryType || this.$route.query.type) === "sec" && list && list.length) {
        this.syncTableListByType("sec");
        this.initCoinInfo("sec");
      }
    },
    spotCoinList(list) {
      if ((this.queryType || this.$route.query.type) === "spot" && list && list.length) {
        this.syncTableListByType("spot");
        this.initCoinInfo("spot");
      }
    },
    contractCoinList(list) {
      if ((this.queryType || this.$route.query.type) === "usta" && list && list.length) {
        this.syncTableListByType("usta");
        this.initCoinInfo("usta");
      }
    },
  ...
  methods: {
    ...
    initCoinInfo(type) {
      ...
      // ✅ 关键：拿到列表并做空保护
      const list = this[typeCoinList] || [];
      if (!list.length) {
        // 列表还没加载好，先别往下执行（否则 coinSymbolInfo 会变 undefined）
        return;
      }

      // ✅ 关键：symbol 兜底
      const symbol = (this.symbol || this.$route.query.symbol || "btc").toLowerCase();

      // ✅ 找币：找不到就用 btc，再找不到就用第一个
      const matchedItem =
        list.find((item) => item.coin === symbol) ||
        list.find((item) => item.coin === "btc") ||
        list[0];
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          typeCoinList,
          symbol: this.coinSymbolInfo.coin || symbol,
        })
      );

      // ✅ 下面开始做类型相关初始化前，先判空
      const newcoin = this.coinSymbolInfo;

      if (type === "sec") {
        // coinSymbolInfo 没 id 就别调
        if (newcoin && newcoin.id) {
          this.getCycleList(newcoin.id);
        }
      } else if (type === "usta") {
        // ✅ 切换时一定要清空，不然会越 push 越多
        this.numList = [];

        if (newcoin && newcoin.leverage) {
          newcoin.leverage.split(",").forEach((ele) => {
            this.numList.push({
              name: ele + "X",
              id: ele,
          ...
          this.transactionNum = this.numList[0] ? this.numList[0].name : null;
        } else {
    ...
    searchCoin(value) {
      this.syncTableListByType(this.queryType || "sec");
      if (value) {
        const v = value.toLowerCase();
        this.tabCoinList = (this.tabCoinList || []).filter(item => (item.coin || "").includes(v));
      }
    },

    initALLQuotesData() {
      if (this.tabsValue == "sec") {
        this.tableList = this.secondContractCoinList;
      } else if (this.tabsValue == "spot") {
        this.tableList = this.spotCoinList;
      } else {
        this.tableList = this.contractCoinList;
      }
   ...
   // 行情切换
    tabsClick(e) {
      const nextType = e.name; // sec / spot / usta
      // 当前选中的币，兜底 btc
      // const symbol = (
      //   (this.coinSymbolInfo && this.coinSymbolInfo.coin) ||
      ...
      // 同一个路由就别重复 push（可选）
      // if (this.$route.query.type === nextType && this.$route.query.symbol === symbol) {
      //   // 仍然保证列表同步一次（防止某些情况下 tableList 没刷新）
      //   this.syncTableListByType(nextType);
      //   return;
      // }
      this.syncTableListByType(nextType);
      this.$router.push({
        path: "/trade/index",
        query: { type: nextType },
        // query: { type: nextType, symbol },
      });
    },

    syncTableListByType(type) {
      // console.log("type",type);
      // console.log(" this.secondContractCoinList", this.secondContractCoinList);
      // console.log(" this.spotCoinList", this.spotCoinList);
      // console.log(" this.contractCoinList", this.contractCoinList);
      ...
      this.tabCoinList =  this.tableList
    },
...
<style lang="scss" scoped>
  ...
  .spot_content,
  .bottom_input{
    display: flex; // 你如果依赖 flex-between，这行可不要，但建议加上更稳
  }
