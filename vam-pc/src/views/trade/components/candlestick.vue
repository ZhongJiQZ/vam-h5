...
import { _add, _div, _mul, priceFormat } from "@/util/utildecimal";
...
export default {
  ...
  watch: {
    coinInfo: {
      ...
      immediate: true,
      handler(v) {
        if (!v || !v.symbolUpperCase) return;

        // 第一次创建
        if (!this.widget) {
          Object.assign(this.currentCoinInfo, v);
          this.supportedResolutions = this.getSupportedResolutions();
          this.headIntervalList.splice(0, this.headIntervalList.length, ...this.getHeadIntervalList());
        ...
        // 已存在 widget：切换币种时更新
        this.setSymbol(v.symbolUpperCase, this.currentInterval.interval, () => {
          Object.assign(this.currentCoinInfo, v);
        });
      }
    }
  ...
  mounted() {
    // console.log("candle stick coinInfo====>", this.coinInfo);
