...
/*
  ...
  export const financialSubmit = (data) => {
     console.log("data==>",data);
    return request({
      url: `/api/financial/submit?planId=${data.planId}&money=${data.money}&days=${data.days}`,
      method: 'post'
    })
  }
...
export const getInvestmentList = (data) => {
  return request({
    url: `/api/order/list?pageNum=${data.pageNum}&pageSize=${data.pageSize}`,
    method: 'post'
  })
}
